	...
	describe('setupFetch', () => {
		describe('getOne', () => {
			it('getOne with response', async () => {
				const spy = vi.spyOn(global, 'fetch').mockResolvedValueOnce({ ok: true, status: 200, text: () => Promise.resolve(JSON.stringify({ something: 'anything' })) } as any)
		...
		describe('delOne', () => {
			it('delOne without request body and response', async () => {
				const spy = vi.spyOn(global, 'fetch').mockResolvedValueOnce({ ok: true, status: 200, text: () => Promise.resolve(null) } as any)
